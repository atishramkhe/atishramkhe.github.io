      ...
      if (section.sourceSectionId) {
        ...
        if (section.sourceSectionId === 'non-english') {
          ...
          if (section.id === 'anime-streaming') {
            const featuredAnimeNames = ['AnimeKai', 'HiAnime', 'KickAssAnime', 'otakuu 2'];
            const newAnimeSites = [];
            ...
            const featuredSportsNames = ['Streamed 2', 'PPV.TO', 'TimStreams'];
            ...
            const featuredLiveTVNames = ['AlienFlix TV', 'NTV', 'DaddyLive TV', 'WiTV','DistroTV'];
            ...
            if (!filteredLinks.some(site => site.name === 'WiTV')) {
              filteredLinks.push({ name: 'WiTV', url: 'https://witv.soccer/' });
            }
...
function injectLoadingStyles() {
  ...
  const css = `
    .fmhy-loading-overlay {
      ...
      align-items: center;
      justify-content: center;
      background: url('/homepage/assets/background_5280px.png') no-repeat center center fixed, radial-gradient(circle at center, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.95) 60%), #000;
