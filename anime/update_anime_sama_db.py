...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    existing_columns = {
        row[1] for row in conn.execute("PRAGMA table_info(anime)")
    }
    desired_columns = {
        "last_seen_at": "TEXT",
...
def bootstrap_metadata(conn: sqlite3.Connection, now_iso: str) -> None:
    rows = conn.execute(
        "SELECT id, slug FROM anime WHERE refresh_bucket IS NULL"
    ).fetchall()
    if rows:
        conn.executemany(
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_map = {
            executor.submit(scrape_slug, slug, base_url): slug
